...
import { BookOpen, ArrowLeft, ClipboardList, ExternalLink, FlaskConical, Layers3, Search, Waypoints } from "lucide-react";
...
import { getOrInitCard, isDue, loadSrsState, reviewCard, saveSrsState, type Grade } from "@/lib/srs";
import {
...
function FurtherReading({ topicName, color }: { topicName: string; color: { bg: string; fg: string } }) {
  ...
  const links = [
    ...
    { label: "NCBI Bookshelf", sub: "free full-text physiology textbooks", href: `https://www.ncbi.nlm.nih.gov/books/?term=${q}` },
    { label: "LibreTexts Medicine", sub: "open-access physiology course text", href: `https://med.libretexts.org/Search?query=${q}` },
  ];
  return (
...
const TOPIC_PEARLS: Record<string, string> = {
  general: "Facilitated diffusion is saturable (unlike simple diffusion) because it depends on a finite number of carrier proteins — a favorite way examiners test the difference between the two.",
  blood: "Rh-negative mothers receive anti-D immunoglobulin (RhoGAM) during and after pregnancy specifically to prevent sensitization that would endanger a future Rh-positive fetus.",
  "nerve-muscle": "Myasthenia gravis (postsynaptic, nicotinic ACh receptor antibodies) causes fatigable weakness that worsens with activity, while Lambert-Eaton syndrome (presynaptic, voltage-gated Ca2+ channel antibodies) classically improves with repeated activity.",
  ...
  "special-senses": "Presbyopia (lens elasticity loss with age) is a different mechanism entirely from myopia/hyperopia (eyeball shape/refractive errors) — a common exam mix-up.",
  ...
  respiratory: "The Bohr effect (right-shifted O2-Hb curve with rising CO2/falling pH) favors oxygen unloading exactly where it's needed most: actively metabolizing, CO2-producing tissue.",
  renal: "Creatinine clearance is preferred over BUN for estimating GFR because creatinine production is relatively constant and minimally affected by diet or hydration, unlike urea.",
  ...
  endocrine: "A high TSH with low T4 (primary hypothyroidism) and a low TSH with low T4 (secondary/pituitary hypothyroidism) look similar in symptoms but have opposite TSH results — always interpret TSH and T4 together.",
  reproductive: "The mid-cycle LH surge is one of physiology's only examples of positive feedback — sustained high estrogen flips from suppressing to stimulating GnRH/LH release.",
};

const GALLERY_ICONS = [
  { src: "/icons/heart.svg", label: "Human heart (cross-section)", caption: "Chambers, valves, and great vessels — the classic labeled anatomy reference.", topicId: "cvs" },
  { src: "/icons/neuron.svg", label: "Pyramidal neuron", caption: "Dendritic tree, soma, and axon — the basic signaling unit of the CNS.", topicId: "cns" },
  { src: "/icons/cardiomyocyte.svg", label: "Cardiomyocyte", caption: "Cardiac muscle cell — striated, but functions as an electrical syncytium via gap junctions.", topicId: "cvs" },
  { src: "/icons/nephron.svg", label: "Nephron", caption: "The functional unit of the kidney — filtration, reabsorption, secretion.", topicId: "renal" },
  { src: "/icons/red-blood-cell.svg", label: "Red blood cell", caption: "Biconcave, no nucleus or mitochondria — maximizes surface area for gas exchange.", topicId: "blood" },
  { src: "/icons/smooth-muscle-cell.svg", label: "Smooth muscle cell", caption: "Spindle-shaped, involuntary — lines the GI tract, blood vessels, and airways.", topicId: "gi" },
  { src: "/icons/sperm.svg", label: "Sperm", caption: "Haploid male gamete — motile via a flagellum powered by mitochondria in the midpiece.", topicId: "reproductive" },
  { src: "/icons/oocyte.svg", label: "Oocyte (MII)", caption: "Arrested in metaphase II until fertilization triggers completion of meiosis.", topicId: "reproductive" },
];

const DIAGRAMS: { topicId: string; title: string; description: string; pearl: string; Component: () => ReactNode }[] = [
  { topicId: "nerve-muscle", title: "Nerve Action Potential", description: "Depolarization, repolarization, and the refractory period, phase by phase.", pearl: "Local anesthetics (e.g. lidocaine) work by blocking voltage-gated Na+ channels, preventing the depolarization phase from ever reaching threshold.", Component: ActionPotentialDiagram },
  { topicId: "nerve-muscle", title: "Sliding Filament / Cross-Bridge Cycle", description: "How Ca2+ and ATP drive the myosin-actin cross-bridge cycle.", pearl: "Rigor mortis happens because ATP depletion after death leaves myosin permanently bound to actin — ATP is needed to release the cross-bridge, not just to power it.", Component: SlidingFilamentDiagram },
  { topicId: "cns", title: "Stretch Reflex Arc", description: "The monosynaptic pathway behind the knee-jerk reflex.", pearl: "Because it's monosynaptic, the stretch reflex is one of the fastest reflex pathways in the body — clinically tested as deep tendon reflexes (DTRs).", Component: ReflexArcDiagram },
  { topicId: "cvs", title: "Cardiac Cycle", description: "The five phases from atrial systole to ventricular filling.", pearl: "A third heart sound (S3) in a young healthy adult can be normal, but in an older adult it often signals volume overload — such as heart failure.", Component: CardiacCycleDiagram },
  { topicId: "cvs", title: "Cardiac Conduction System", description: "SA node to Purkinje fibers — and why the AV node delays the signal.", pearl: "If the SA node fails, the AV node can take over as a backup pacemaker, but at a slower intrinsic rate — the basis of a junctional escape rhythm.", Component: CardiacConductionDiagram },
  { topicId: "cvs", title: "Baroreceptor Reflex", description: "The fast negative feedback loop that buffers acute blood pressure swings.", pearl: "Standing up suddenly triggers this reflex to prevent orthostatic hypotension — a blunted baroreflex (common with aging or autonomic disease) causes dizziness on standing.", Component: BaroreceptorReflexDiagram },
  { topicId: "cvs", title: "Frank-Starling Curve", description: "How preload determines stroke volume, and how contractility shifts the whole curve.", pearl: "In heart failure, the Frank-Starling curve flattens and shifts down-right — the same increase in preload produces a smaller rise in stroke volume than normal.", Component: FrankStarlingDiagram },
  { topicId: "respiratory", title: "Oxygen-Hemoglobin Dissociation Curve", description: "The sigmoid curve, and what shifts it right (Bohr effect) or left.", pearl: "Fetal hemoglobin (HbF) has a left-shifted curve compared to adult HbA, giving it higher O2 affinity — helping the fetus extract oxygen from maternal blood across the placenta.", Component: OxyHemoglobinCurveDiagram },
  { topicId: "renal", title: "Nephron Segments & Function", description: "What each part of the nephron actually does to the filtrate.", pearl: "Loop diuretics (e.g. furosemide) block the Na-K-2Cl transporter in the thick ascending limb, disrupting the countercurrent multiplier and impairing the kidney's ability to concentrate urine.", Component: NephronSegmentsDiagram },
  { topicId: "endocrine", title: "HPA Axis", description: "Hypothalamus → pituitary → adrenal cortex, and the negative feedback that closes the loop.", pearl: "Long-term exogenous steroid use suppresses the HPA axis via negative feedback — abruptly stopping steroids can cause adrenal insufficiency because the axis needs time to 'wake back up.'", Component: HpaAxisDiagram },
  { topicId: "reproductive", title: "Menstrual Cycle Hormone Phases", description: "Follicular phase, ovulation, luteal phase, and menstruation.", pearl: "Combined oral contraceptives work partly by suppressing the LH surge — without it, ovulation is prevented even though follicles may start developing.", Component: MenstrualCycleDiagram },
];
...
function Diagrams() {
  ...
  return (
    <section>
      <SectionIntro eyebrow={`${DIAGRAMS.length} animated diagrams`} title="See the systems move" description="Original diagrams grouped by chapter, each with a clinical pearl. Tap any diagram or picture to enlarge it." />
              ...
              {iconsInUnit.length > 0 && (
                <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4">
                  {iconsInUnit.map((icon) => {
                    ...
                    return (
                      <button key={icon.src} type="button" onClick={() => setOpenIcon(icon)} style={{ borderTopColor: color.ring }} className="group rounded-xl border border-border border-t-4 bg-card p-3 text-center shadow-sm transition hover:-translate-y-1 hover:shadow-lg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring active:scale-95">
...
function FactSheets() {
  ...
  if (!topicId) {
    ...
    return (
      <section>
        <SectionIntro eyebrow={`${FACTS.length} high-yield facts`} title="Choose a system" description="Organized by unit — the sequence most Indian MBBS physiology courses teach from." />
...
function NormalValues() {
  ...
  const results = useMemo(() => {
    ...
    return NORMAL_VALUES.filter((v) => v.parameter.toLowerCase().includes(q) || v.note.toLowerCase().includes(q));
  }, [query]);
  const [openValue, setOpenValue] = useState<(typeof NORMAL_VALUES)[number] | null>(null);

  return (
    <section>
      <SectionIntro eyebrow={`${NORMAL_VALUES.length} normal values`} title="Normal Values Reference" description="Search by parameter name. Tap any card for the full clinical note." />
      <div className="relative mb-3">
        <Search aria-hidden="true" size={18} className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground" />
        <label className="sr-only" htmlFor="value-search">Search normal values</label>
        <input id="value-search" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search heart rate, GFR, TSH…" className="min-h-12 w-full rounded-xl border border-input bg-card pl-10 pr-4 text-sm text-foreground shadow-sm outline-none placeholder:text-muted-foreground focus:ring-2 focus:ring-ring" />
      </div>
      <p aria-live="polite" className="mb-4 text-xs font-medium text-muted-foreground">Showing {results.length} of {NORMAL_VALUES.length} values</p>
        ...
        {results.map((v) => {
          ...
          return (
            <button key={v.id} type="button" onClick={() => setOpenValue(v)} style={{ borderTopColor: color.ring }} className="group rounded-xl border border-border border-t-4 bg-card p-4 text-left shadow-sm transition hover:-translate-y-0.5 hover:shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring active:scale-[0.98]">
              ...
              <p style={{ backgroundColor: color.bg, color: color.fg }} className="mt-2 inline-block rounded-md px-2 py-0.5 font-mono text-xs">{v.value}</p>
              <p className="mt-2 line-clamp-2 text-xs leading-snug text-muted-foreground">{v.note}</p>
            </button>
          );
        })}
        {results.length === 0 && <p className="col-span-2 py-12 text-center text-sm text-muted-foreground">No value matches "{query}".</p>}
      </div>
      {openValue && (
        <Modal title={openValue.parameter} color={topicColor(openValue.topicId)} onClose={() => setOpenValue(null)}>
          ...
          <p style={{ backgroundColor: topicColor(openValue.topicId).bg, color: topicColor(openValue.topicId).fg }} className="mt-3 inline-block rounded-md px-3 py-1 font-mono text-sm">{openValue.value}</p>
    ...
    <div className="flex min-h-screen flex-col bg-background">
      ...
      <main className="mx-auto flex w-full max-w-4xl flex-1 flex-col px-4 py-10 sm:px-6 sm:py-16">
        <div className="relative overflow-hidden rounded-3xl bg-[linear-gradient(135deg,oklch(0.94_0.08_300),oklch(0.93_0.07_195)_45%,oklch(0.94_0.1_75))] p-6 text-center shadow-sm sm:p-12">
          ...
          <h1 className="relative mt-5 font-display text-4xl leading-tight text-foreground sm:text-5xl">Physiology High-Yield</h1>
          <p className="relative mx-auto mt-3 max-w-lg text-sm leading-relaxed text-muted-foreground sm:text-base">
            Fact sheets, flashcards, animated system diagrams, MCQ practice, and a normal values reference — chaptered the way Indian MBBS students study, with clinical pearls throughout.
          </p>
...
export default function App() {
  ...
  if (!entered) return <LandingPage onEnter={(t) => { setTab(t); setEntered(true); }} />;
      ...
      <header className="sticky top-0 z-20 border-b border-border bg-background/95 backdrop-blur">
        <div className="mx-auto flex w-full max-w-4xl flex-col gap-3 px-4 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-display text-2xl leading-none text-foreground">Physiology High-Yield</h1>
          ...
          <nav aria-label="Study modes" className="grid grid-cols-4 rounded-xl bg-muted p-1">
            ...
            <TabButton active={tab === "values"} onClick={() => setTab("values")} icon={<FlaskConical size={16} />} label="Normal Values" />
          </nav>
        </div>
      </header>
